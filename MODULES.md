...
| **Daisy MultiFX** | `eurorack_daisy_patch_init` | `daisy_multifx_oled/` | Daisy Patch Submodule | `daisy-multifx.md`. **The unit is repurposed from 2026-09-24** for the neural-model experiment (`nam_patchinit/`); re-flash `daisy_multifx_oled/` to restore it. |
...
| **Alchemy Lab** | Hermetic Modular | 12 ✓ | open DSP platform | **Electrosmith Daisy inside** (Daisy Seed 2 DFM — STM32H750, 64 MB SDRAM), MIT-licensed SDK, stereo audio in/out, six pots, three buttons, six firmware-controllable CV jacks, USB-C MIDI, microSD, 102 addressable LEDs, no screen. The fallback host for `nam_patchinit/` if it outgrows the patch.init's CV. |
